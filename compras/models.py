...
class Compra(models.Model):
    ...
    # Identificación
    numero_compra = models.CharField(max_length=20, editable=False, null=True, blank=True)
    ...
    class Meta:
        ...
        # ✅ correlativo único por empresa
        constraints = [
            models.UniqueConstraint(fields=['empresa', 'numero_compra'], name='uniq_numero_compra_por_empresa')
        ]
    ...
    def save(self, *args, **kwargs):
        # ✅ número correlativo por empresa: COM-000001
        if not self.numero_compra:
            ultima = Compra.objects.filter(empresa=self.empresa).order_by('-id').first()
